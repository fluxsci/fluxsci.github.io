...
<header class="site-header">
  <div class="header-inner">
    ...
    >
      ...
      <a href="#only-flux">Only in Flux</a>
...
<main id="main-content" class="flux-home">
  <section class="hero" aria-labelledby="hero-title">
    <div class="hero-copy content-width">
      ...
      <p class="eyebrow intro-rise" style="--intro-delay: 260ms">
        <span class="eyebrow-line"></span>A desktop studio for scientific work
      </p>
      <h1 id="hero-title" class="intro-rise" style="--intro-delay: 340ms">
        One studio for your<br /><em>scientific work.</em>
      </h1>
      <p class="hero-description intro-rise" style="--intro-delay: 430ms">
      ...
      <div class="hero-actions intro-rise" style="--intro-delay: 520ms">
        <a
    ...
    <figure class="hero-figure wide-width intro-rise" style="--intro-delay: 640ms">
  ...
  <section
    id="only-flux"
    class="signature-section section-space"
    aria-labelledby="signature-title"
  >
    <div class="content-width">
      <div class="signature-heading">
        <p class="eyebrow">Only in Flux</p>
        <h2 id="signature-title">
          Every mark has a name.<br /><em>Everything else follows.</em>
        </h2>
        <p>
          A plot made with fluxplot keeps its meaning: each series, point, axis
          and label is a named part of the file. That one idea is what lets
          Flux edit a figure without touching the analysis, animate it part by
          part in a slide, keep your styling when the data regenerate, and let
          an agent point at exactly the part you mean.
        </p>
      </div>
      <div
        class="xray"
      ...
      >
        <div class="xray-stage-wrap">
          <div class="xray-topline">
            <span
              ><span class="status-dot"></span>A real plot file from the example
              project, read by your browser</span
        ...
        <p class="xray-caption">
          Nothing here is staged: the page loads the project’s own SVG and reads
          its <code>data-role</code> and <code>data-series</code> attributes.
          Rerun the analysis at another frequency and the same named points and
          curves move to their new values while your restyle stays attached,
          which is exactly how a Flux figure treats a regenerated plot.
        </p>
      </div>
      <div class="signature-stories">
        <article class="story" data-reveal>
          <div class="story-media story-media--diagram">
            <svg
              class="surfaces-diagram"
              viewBox="0 0 600 236"
              role="img"
              aria-labelledby="surfaces-title surfaces-desc"
            >
              <title id="surfaces-title">One plot, three surfaces</title>
              <desc id="surfaces-desc">
                The same named plot panel in a figure, sent to a slide where it
                gains animation steps, and inserted into a paper where it stays
                interactive.
              </desc>
              <defs>
                <pattern id="surfaces-heat" width="6" height="6" patternUnits="userSpaceOnUse">
                  <rect width="6" height="6" fill="#dce6e3" />
                  <rect width="3" height="3" fill="#8fbfba" />
                  <rect x="3" y="3" width="3" height="3" fill="#2f7f79" />
                </pattern>
              </defs>
              <g class="surface">
                <rect x="14" y="26" width="160" height="112" rx="4" />
                <text x="26" y="47" class="letter">c</text>
                <rect x="44" y="44" width="112" height="72" fill="url(#surfaces-heat)" class="panel" />
                <path d="M44 116h112M44 44v72" class="axis" />
                <text x="14" y="164" class="label">Figure 1 · panel c</text>
                <text x="14" y="180" class="sub">selected: cell-responses.x-heatmap</text>
              </g>
              <g class="link">
                <path d="M180 82h34" />
                <path d="M210 77l6 5-6 5" />
                <text x="197" y="68" class="link-label" text-anchor="middle">Send to deck</text>
              </g>
              <g class="surface">
                <rect x="222" y="26" width="160" height="86" rx="4" class="stage" />
                <rect x="248" y="38" width="96" height="60" fill="url(#surfaces-heat)" class="panel" />
                <path d="M248 98h96M248 38v60" class="axis" />
                <text x="356" y="94" class="letter" text-anchor="end">c</text>
                <g class="beats">
                  <rect x="222" y="120" width="46" height="16" rx="3" />
                  <rect x="274" y="120" width="46" height="16" rx="3" class="beat-active" />
                  <rect x="326" y="120" width="56" height="16" rx="3" />
                  <text x="245" y="131" text-anchor="middle">1</text>
                  <text x="297" y="131" text-anchor="middle" class="beat-active-label">2</text>
                  <text x="354" y="131" text-anchor="middle">3</text>
                </g>
                <text x="222" y="164" class="label">Slide · step 2 of 3</text>
                <text x="222" y="180" class="sub">same parts, plus a timeline</text>
              </g>
              <g class="link">
                <path d="M388 82h34" />
                <path d="M418 77l6 5-6 5" />
                <text x="405" y="68" class="link-label" text-anchor="middle">Insert slide</text>
              </g>
              <g class="surface">
                <rect x="430" y="26" width="156" height="112" rx="4" />
                <path d="M444 44h128M444 54h104M444 64h118" class="prose" />
                <rect x="470" y="74" width="76" height="46" fill="url(#surfaces-heat)" class="panel" />
                <path d="M470 120h76M470 74v46" class="axis" />
                <path d="M444 130h128" class="prose" />
                <text x="430" y="164" class="label">Paper · still interactive</text>
                <text x="430" y="180" class="sub">HTML plays the steps; PDF keeps step 0</text>
              </g>
              <text x="14" y="222" class="foot">One object, three surfaces. Every part keeps its name across all of them.</text>
            </svg>
          </div>
          <div class="story-copy">
            <span class="story-index">01</span>
            <h3>A slide is a figure with a timeline.</h3>
            <p>
              Send a figure to a deck and you are still on the same canvas, with
              the same tools and the same named parts; Flux adds steps. Reveal
              the heatmap, draw the network edges on, bring in the
              interpretation, then place that slide back into the paper, where
              it keeps playing.
            </p>
            <a class="text-link" href="#slides"
              >Step through a real slide below<svg class="icon" aria-hidden="true">
                <use href="#i-arrow" /></svg
            ></a>
          </div>
        </article>
        <article class="story" data-reveal style="--reveal-delay: 120ms">
          <div
            class="story-media story-terminal"
            role="img"
            aria-label="Three Flux commands: restyle a named series in a figure, animate a named network part so its edges draw on during a slide step, and render the figure to PNG."
          >
            <div><span class="comment"># the same names an agent or a script can use</span></div>
            <div><span class="prompt">$</span> flux restyle fig-neural-populations direction-90-cells.points --fill '#205ea6'</div>
            <div><span class="prompt">$</span> flux set-animation neural-populations results relationships --target response-network --part signal-correlation-edges.x-similarity-edges --preset drawOn</div>
            <div><span class="prompt">$</span> flux render-figure fig-neural-populations --png</div>
          </div>
          <div class="story-copy">
            <span class="story-index">02</span>
            <h3>Point at the part you mean, from anywhere.</h3>
            <p>
              The ids in the file are the ids in the app, on the command line,
              and over MCP. An agent working in your project restyles the
              series you name, animates the edges you name, and renders the
              result for you to review in the same workspace where you write.
            </p>
            <a class="text-link" href="#agents"
              >How agents work in Flux<svg class="icon" aria-hidden="true">
                <use href="#i-arrow" /></svg
            ></a>
          </div>
        </article>
